	...
	int structureChecker (int lower48, int structure, int MC, DoublePos boxDimensions, StructData data[], int dataSize, int result);

	const int r = 10;
	const DoublePos origCoords = {{-10, -10}, {10, 10}};
	...
	//return an integer - if 0, then false. if 1, then true
	int structureChecker (int lower48, int structure, int MC, DoublePos boxDimensions, StructData data[], int dataSize, int result)
	{
		Pos p;
		int i = 0;
	...
	int main() 
	{
		...
		int i = 0;
		DoublePos boxDimensions;
		boxDimensions.first.x = origCoords.first.x - r;   // Update boxDimensions for the first structure
		boxDimensions.first.z = origCoords.first.z - r;
		boxDimensions.second.x = origCoords.second.x + r;
		boxDimensions.second.z = origCoords.second.z + r;
		...
		boxDimensions.first.x = data[i].regionCoords.first.x - r;
		boxDimensions.first.z = data[i].regionCoords.first.z - r;
		boxDimensions.second.x = data[i].regionCoords.second.x + r;
		boxDimensions.second.z = data[i].regionCoords.second.z + r;
		...
		for (uint64_t lower48 = START_STRUCTURE_SEED; lower48 < STRUCTURE_SEEDS_TO_CHECK; ++lower48) 
		{
			...
			result = structureChecker(lower48, Bastion, MC, boxDimensions, data, numberOfStructs, result);
			if (result == 1) 
			{
